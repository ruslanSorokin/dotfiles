...
let completer_matcher = {|spans|
    ...
    let complete_in_fish: list<string> = [
      asdf
      mise
      yazi
      zellij
    ...
    match $spans.0 {
        ...
        git => $fish_completer
        # carapace doesn't have completions for these
        $a if ($a in $complete_in_fish) => $fish_completer
...
$env.config = {
    ...
    completions: {
        ...
        external: {
            ...
            completer: $completer_matcher # check 'carapace_completer' above as an example
        }
...
source ($nu.default-config-dir | path join "functions/util.nu")
