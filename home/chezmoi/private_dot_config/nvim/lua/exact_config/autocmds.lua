...
local function chezmoi_config(filename, lang)
  ...
  if is_chezmoi_config then
    local ft, callback = gotmpl_composite_ft_and_buf_callback(lang)
    callback()
    vim.bo.filetype = ft
  end
...
autocmd("FileType", {
  ...
  callback = function(args)
    vim.schedule(function()
      local lang = vim.bo.filetype:match("(.*)%.chezmoitmpl")
      lang = chezmoi_ft_overrides[lang]

      if chezmoi_config(args.file, lang) then
        return
      end

      if gotmpl_directly_injectable(lang) then
        return
      end
